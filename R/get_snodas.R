...
# Zach Hoylman 1-22-2020, modified 9-11-2020, modified 2-13-2021
...
# finally this script clips the data to a domain of interest and deletes all intermediary files
...
get_snodas = function(date, domain, domain_name){
  for(d in 1:length(date)){
    ...
    tar.dir = paste0("/home/zhoylman/water-balance-data/SNODAS/raw/SNODAS_", date[d] %>% format(., "%Y%m%d"), ".tar")
    unzip.dir = paste0("/home/zhoylman/water-balance-data/SNODAS/raw/SNODAS_", date[d] %>% format(., "%Y%m%d"))
    ...
    files_of_interest = c("1025")
    
    for(i in 1:length(files_of_interest)) {
      file_to_process = files[[which(files %like% files_of_interest[i] 
                                     & files %like% ".dat.gz" 
                                     & files %like% 'L00')]]
...
byte order = 1", con = file_to_process %>% gsub(".dat.gz", ".hdr", .))
      ...
      processed_name = paste0("~/water-balance-data/SNODAS/precipitaion/",
                              if(files_of_interest[i] == "1025") paste0("snodas_precip_conus_", format(date[d],"%Y%m%d"), ".tif") 
                              else NA)
      
      processed_name_domain = paste0("~/water-balance-data/SNODAS/precipitaion/",
                              if(files_of_interest[i] == "1025") paste0("snodas_precip_",domain_name,'_', format(date[d],"%Y%m%d"), ".tif") 
                              else NA)
      ...
      raster = raster(processed_name) %>%
        crop(., domain) %>%
        mask(., domain)
      
      writeRaster(raster, processed_name_domain, overwrite=TRUE)
    } 
    
    #erase raw data so we dont over store
    system(paste0("rm -r ", tar.dir))
    system(paste0("rm -r ", unzip.dir))
    system(paste0("rm ", processed_name))
  }
}
